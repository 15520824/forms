<?php
    function write_reporter_survey_perform_script(){
?>
<script type="text/javascript">
"use strict";

formTest.reporter_survey_perform.generateData = function(host) {
    var data, header;
    DOMElement.removeAllChildren(host.survey_container_container);
};

formTest.reporter_survey_perform.init = function(host, mode) {
    host.database = {};
    host.root_survey = 0;
    ...
    Promise.all(promiseAll).then(function() {
        ...
        switch (mode) {
            case 1:
                formTest.reporter_survey_perform_information.init(mainFrame.$viewport, host)
                break;
            case 2:
                break;
            case 3:
